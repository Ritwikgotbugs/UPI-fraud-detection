...
import { DollarSign, CreditCard, Activity, Zap, Bell } from 'lucide-react';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (userData?.upiId) {
      ...
      setBalance(Math.floor(50000 + Math.random() * 50000));

      // Fetch transactions for current user
      const fetchTransactions = async () => {
        try {
          const txRef = collection(db, "transactions");
          const txQuery = query(txRef, where("senderUPI", "==", userData.upiId));
          const txSnapshot = await getDocs(txQuery);
          const txList = txSnapshot.docs.map(d => ({
            id: d.id,
            ...d.data(),
          }));
          ...
          months.forEach((m, i) => {
            monthlyMap[m] = Math.floor(300 + Math.random() * 700);
          });
          setMonthlySpending(months.map(m => ({ name: m, value: monthlyMap[m] })));

          // Create notifications from latest transaction
          if (txList.length > 0) {
            const latest = txList[0];
            setNotifications([
              {
                id: 1,
                message: `Transaction to ${latest.recipientUPI} - ₹${latest.amount}`,
                time: "Just now",
